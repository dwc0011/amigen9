...
function MonitorAgentSetup {
    ...
    # Azure Log Analytics Agent is not supported on EL9 when FIPS mode is active
    if [[
      $( chroot "${CHROOTMNT}" /bin/bash -c "fips-mode-setup --check" ) == \
      "FIPS mode is enabled."
    ]]
    then
      STATUS_MSG="Azure Monitor Agent not supported on EL9"
...
function WaagentSetup {
  ...
  # EL9-specific guidance has been released by the Azure team
